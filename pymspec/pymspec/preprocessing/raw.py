...
class QualityControl(PreprocessorMixin):
    ...
    def transform(self, spectra_list):
        mini = max([0, self.average_intensity - self.restriction_ratio * self.std])  # In some rare case the value was under 0
        maxi = self.average_intensity + self.restriction_ratio * self.std
        qc_passed = []
        for spectrum in spectra_list:
            intensity_sum = np.sum(spectrum.intensity_values)
            if mini > intensity_sum or intensity_sum > maxi:  # Does not pass QC
