...
from pico.domain import DataMatrix
...
def test_givenData_whenLoadData_thenDataIsLoaded(
    open_mock, pickle_mock, input_data_matrix
):
...
def test_givenData_whenLoadSampleWithEmptyList_thenNoDataIsLoaded(input_data_matrix):
    ...
    assert input_data_matrix.load_samples_corresponding_to_IDs_in_splits([]).equals(
        DATAMATRIX_DATAFRAME.loc[[], :]
    )
...
def test_givenData_whenLoadSampleWithIdList_thenTheDataIsLoaded(input_data_matrix):
    ...
    assert input_data_matrix.load_samples_corresponding_to_IDs_in_splits(
        selected_samples
    ).equals(DATAMATRIX_DATAFRAME.loc[selected_samples, :])


@patch("sklearn.preprocessing.StandardScaler.transform", return_value=SCALED_DATA)
def test_givenData_whenGettingScaleData_thenTheScaleDataIsLoaded(
    scaler_mock, input_data_matrix
):
    input_data_matrix.data = DATAMATRIX_DATAFRAME
