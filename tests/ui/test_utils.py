import pico.conf.parameters as cfg
import pico.ui.tabs.utils
...
def test_update_marks():
    ...
    # Test case where the custom value is before the default marks
    value = first_value - 1
    marks, location = pico.ui.tabs.utils.update_marks(custom_value=value, add_all_value=False)
    ...
    value = first_value - 1
    marks, location = pico.ui.tabs.utils.update_marks(custom_value=value, add_all_value=True)
...
def test_get_index_from_marks():
    ...
    index = pico.ui.tabs.utils.get_index_from_marks(location, marks)
    assert index == -1
