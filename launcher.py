...
logging.basicConfig(level=logging.INFO, filename='MeDIC_Installation.log', filemode='w',
...
class Loader:
    def __init__(self, desc="Loading...", end="checked", fail="fail",
        ...
        self.steps = ["|MeDIC installation ...     |", "| MeDIC installation ...    |", "|  MeDIC installation ...   |",
                      "|   MeDIC installation ...  |", "|    MeDIC installation ... |", "|     MeDIC installation ...|",
                      "|      MeDIC installation ..|", "|.      MeDIC installation .|", "|..      MeDIC installation |",
                      "|...      MeDIC installation|", "| ...      MeDIC installatio|", "|n ...      MeDIC installati|",
                      "|on ...      MeDIC installat|", "|ion ...      MeDIC installa|", "|tion ...      MeDIC install|",
                      "|ation ...      MeDIC instal|", "|lation ...      MeDIC insta|", "|llation ...      MeDIC inst|",
                      "|allation ...      MeDIC ins|", "|tallation ...      MeDIC in|", "|stallation ...      MeDIC i|",
                      "|nstallation ...      MeDIC |", "|installation ...      MeDIC|", "| installation ...      MeDI|",
                      "|C installation ...      MeD|", "|IC installation ...      Me|", "|DIC installation ...      M|",
                      "|EDIC installation ...      |"]

        if not installation:
            self.steps = ["|MeDIC  |", "| MeDIC |", "|  MeDIC|", "|   MeDI|", "|C   MeD|", "|IC   Me|", "|DIC   M|",
                          "|eDIC   |"]
...
def env_dependencies_verification():
    ...
    with open(REQUIREMENT_FILE, 'r') as f:
        line = f.readline()
        while line:
            line = line.strip()  # permet de retirer les retours à la ligne
...
def install_from_github_on_os():
    ...
    # Remove the temporary installation folder if it exists to avoid any error when downloading the project
    try:
...
def startWebPage():
    # Wait 5 seconds to let the server start before opening the browser to avoid browser timeout
    sleep(5)
...
def launch_medic():
    start_web_page_thread = threading.Thread(target=startWebPage)  # Creation of a thread to start the web page
...
def create_medic_conda_env():
    ...
    if not is_medic_env_exist():
        ...
        with Loader(desc="\tCreating medic environment..."):
            create_MeDIC_env()
        internal_loader = Loader(
            desc="\tRe-checking for medic environment...").start()
...
def main():
    ...
    if no_check:
        if not no_launch:
            with Loader(desc="MeDIC running at http://127.0.0.1:5000... or localhost:5000 on Windows",
                        installation=False):
                launch_medic()
        exit(0)
    ...
    with Loader(desc="MeDIC running at http://127.0.0.1:5000... or localhost:5000 on Windows", installation=False):
        launch_medic()
