    ...
    def _registerCallbacks(self) -> None:
        ...
        def get_experiment_statistics(n_clicks, algo, design_name):
            if n_clicks >= 1:
                ...
                return dbc.Table(table_body, id="table_exp_info", borderless=True, hover=True) #dbc.Table.from_dataframe(df, borderless=True)
            else:
                return dash.no_update
        ...
        def show_pca(n_clicks, algo, design_name):
            if n_clicks >= 1:
                ...
                classes = self.r[design_name][algo].results["classes_train"]
                return self._plots.show_PCA(df, classes)
            else:
                return dash.no_update
