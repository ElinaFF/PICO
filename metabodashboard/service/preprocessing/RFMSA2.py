...
class Reference_free_aligner2():
    ...
    def _train(self, train_set, auto_optimizer=False):
        ...
        ppm_at_mz = possible_mz * float(self.max_distance*1) / 10**6  # 1.1x themax distance allowed in a cluster.
        ...
        for start_idx, stop_idx in izip(block_starts, block_ends):
            self._handle_mz_block(possible_mz, start_idx, stop_idx, len(train_set), auto_optimizer)

        self.reference_mz.sort()

    def _handle_mz_block(self, possible_mz, start_idx, stop_idx, num_of_spect, auto_optimizer, problem_reduction=False):
        ...
        if start_idx == stop_idx:
        ...
        elif len(block_mz_values) > 100000 and problem_reduction==False:   #Could be less or more... Not sure I want a parameter...
            modified_block_mz_values = self._handle_mz_block(possible_mz, start_idx, stop_idx, num_of_spect,
                                                             auto_optimizer, True)
            # We want to reduce the number of peak to an acceptable level before performing the full clustering. Kinda tricky...
            # Should we accept to make error because of the problem complexity???
            D = pdist(modified_block_mz_values.reshape(-1, 1), metric="euclidean")

            # Compute the clustering distance threshold
            block_cluster_threshold = np.mean(modified_block_mz_values * self.max_distance/10**6)  # Use mean window size
            ...
            for cluster_id in uniq_clusters:
                ...
                if auto_optimizer:
                    if len(points_in_cluster) == num_of_spect:
        ...
        elif problem_reduction:
            ppm_at_mz = block_mz_values * float(self.max_distance/2) / 10**6
            d_next = np.hstack((np.abs(block_mz_values[:-1] - block_mz_values[1:]), [0]))  # The distance to the next peak
            tmp = np.where(d_next > ppm_at_mz)[0]
            block_ends = np.hstack((tmp, [len(block_mz_values) - 1]))
            block_starts = np.hstack(([0], tmp + 1))
            modified_block_mz_values = []
            for start_idx, stop_idx in izip(block_starts, block_ends):
                sub_block_mz_values = block_mz_values[start_idx:stop_idx+1]
                block_start_mz = block_mz_values[start_idx]
                block_stop_mz = block_mz_values[stop_idx]
                #We are reducing the number of peak
                logging.debug("Computing the distance between each pair of peaks using the %s m/z values" %len(sub_block_mz_values))
                D = pdist(sub_block_mz_values.reshape(-1, 1), metric="euclidean")

                # Compute the clustering distance threshold
                block_cluster_threshold = np.mean(sub_block_mz_values * (self.max_distance/2)/10**6)  # Use mean window size
                logging.debug("A %d ppm interval roughly corresponds to %.6f m/z" % (self.max_distance/2,
                                                                                     block_cluster_threshold))
                ...
                for cluster_id in uniq_clusters:
                    ...
                    points_in_cluster = block_mz_values[cluster_grouper]
                    if len(points_in_cluster) == num_of_spect:
                            self.reference_mz.append(np.round(np.average([mz for mz in points_in_cluster]), decimals=4)) #We know this cluster is good
                    else:
                        [modified_block_mz_values.append(mz) for mz in points_in_cluster]
            return modified_block_mz_values
