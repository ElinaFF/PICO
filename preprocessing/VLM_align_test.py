...
class Point:
    ...
    """
    def __init__(self, mz, intensity, sample=""):
        self.mz = mz
        self.intensity = np.log10(intensity)
        self.sample = sample


class virtual_lock_mass_aligner:
    # TODO: Explain each option and make sure values are ok.
    def __init__(self, min_mz=50, max_mz=1200, clusters_ppm_allowed=10, application_window=10, dist_matrix_dim=1,
    ...
    """
    def auto_optimise_values(self, dataset, possible_cluster_ppm_values_to_test=
    [20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150]):
        """
    #Will optimise cluster_ppm_allowed, overlaping window and application window.
    #The other parameters will remain.
    ##The dataset won't be altered.
    #The VLM parameter will be altered.
    #:param dataset: Dataset used to optimise the parameters. Must contain at least 20 spectra.
    #:return: Nothing
    """
        # Just an idea.. need improvement...

        if len(dataset) < 20:
            raise ValueError("Not enough data to auto-optimise")
        dataset = copy.deepcopy(dataset)
        #np.random.seed(49328)
        test = []
        while len(test) <= len(dataset)/2:
            index = np.random.randint(0, len(dataset)-1)
            test.append(dataset.pop(index))
        number_of_vlm_on_first_round = []
        distance_per_condition = []
        done_enough = False
        for v in possible_cluster_ppm_values_to_test:
            if done_enough:
                break
            vlm = virtual_lock_mass_corrector(clusters_ppm_allowed=v, overlaping_window=v/4)
            vlm.train(dataset)
            number_of_vlm_on_first_round.append(len(vlm.vlm_points))
            if len(number_of_vlm_on_first_round) > 3:
                if number_of_vlm_on_first_round[-3] > number_of_vlm_on_first_round[-2] > number_of_vlm_on_first_round[-1]:
                    done_enough = True
            dist_per_spect = []
            for s in test:
                distance = measure_average_distance_between_two_spect(s, test[0], 1.5*v)
                dist_per_spect.append(distance)
            distance_per_condition.append(dist_per_spect)
    """
    ...
    def apply(self, data):
        for i, spect in enumerate(data):
            data[i] = self._apply_alignment(spect, self.apply_virtual_lock_mass_window)
    ...
    def _plot_dendrogram(self, spectrum, window):
        # Could not work anymore. Need some tests
        window = [float(window[0]), float(window[1])]
    ...
    def _apply_alignment(self, spectrum, window, mode="flat"):
        """
        ...
        v_lockmass, observed_mz = self._find_peak_in_spectra(spect_copy, window)
    ...
    def _find_peak_in_spectra(self, spectrum, window, accept_unfound=True):
        """
        Search for the measured mz. Will return the closest peaks in the window.
        :param window: the m/z area to search. If peak is outside this window, it won't be found.
                       The window is in ppm.
        :param spectrum: The spectrum in which the measured m/z are searched
        :return: The reference masses for which we found a result and the observed mz (from the spectrum)
        ...
        for v_mz in self.vlm_points:
            ...
            if len(possible_matches) == 0:
                if accept_unfound:
                    ...
                    raise ValueError("Peak not found")
            else:  # Will pick the closest peak in the window
                peak = take_closest(possible_matches, v_mz)
            observed_mz.append(np.round(peak, 4))
            mass_found.append(v_mz)
        observed_mz = np.array(observed_mz)
        mass_found = np.array(mass_found)
        np.around(observed_mz, decimals=4)
        return mass_found, observed_mz
    ...
    def confirm_points_are_in_all_spectra(self, spect_list):
        ...
        for spect in spect_list:
            vlm_founds, vlms_real_positions[index] = self._find_peak_in_spectra(spect, window=15.0)
